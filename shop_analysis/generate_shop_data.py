...
# orders의 customer_id는 customers 마스터(4,950명)보다 훨씬 넓은 풀에서 나온다.
# 이는 문제집 3장에서 명시하는 데이터 정합성 이슈(주문에 마스터보다 많은 고객id 등장)를
# 재현하기 위함이며, 동시에 고객당 주문빈도를 현실적인 수준(평균 10회 내외)으로 낮춘다.
ORDER_CUST_POOL = np.arange(1, 20001)  # 마스터의 4,950명은 이 풀의 부분집합

# 고객별 '첫 구매(획득) 월'을 부여 -> 이후 그 달 이전에는 주문이 생기지 않도록 제약
acq_weights = np.array([0.22, 0.18, 0.16, 0.15, 0.15, 0.14])
acq_month_arr = rng.choice(np.arange(1, 7), size=len(ORDER_CUST_POOL), p=acq_weights / acq_weights.sum())
acq_month_map = dict(zip(ORDER_CUST_POOL, acq_month_arr))

# 고객마다 활동성(향후 주문을 낼 가중치)에 편차를 줘서 헤비/라이트 고객이 자연히 갈리게 함
activity_weight = rng.gamma(shape=1.3, scale=1.0, size=len(ORDER_CUST_POOL))
activity_weight = activity_weight / activity_weight.sum()

# 1) 고객마다 첫 구매월에 보장 주문 1건 (코호트 정의를 위해 반드시 존재)
n_guaranteed = len(ORDER_CUST_POOL)
guaranteed_customers = ORDER_CUST_POOL.copy()
guaranteed_months = acq_month_arr.copy()

# 2) 나머지는 각 고객의 첫 구매월 이후(포함)로만, 활동성 가중치로 무작위 배정
n_random = N_ORD - n_guaranteed
random_customers = rng.choice(ORDER_CUST_POOL, size=n_random, replace=True, p=activity_weight)
random_acq = np.array([acq_month_map[c] for c in random_customers])
random_months = rng.integers(low=random_acq, high=7)  # low<=month<=6

order_customer_ids = np.concatenate([guaranteed_customers, random_customers])
months = np.concatenate([guaranteed_months, random_months])

days_in_month = {1: 31, 2: 29, 3: 31, 4: 30, 5: 31, 6: 30}
order_datetimes = []
for m in months:
    d = rng.integers(1, days_in_month[int(m)] + 1)
    h = rng.integers(0, 24)
    mi = rng.integers(0, 60)
    se = rng.integers(0, 60)
    order_datetimes.append(pd.Timestamp(2024, int(m), int(d), int(h), int(mi), int(se)))
